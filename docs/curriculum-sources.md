...
EduForge uses a local, grade-aware planning catalog for the regions shown in the UI. It links teachers to the official framework for final verification; it does not claim to reproduce every local syllabus, state variation, or school sequence. A source-registered pack means the framework boundary is documented; it does **not** mean every subject and outcome has been reviewed.
...
Australia, Singapore, India/CBSE, and IB are source-registered framework packs; they do not yet contain reviewed subject/outcome records. This boundary prevents EduForge from claiming universal official alignment before the underlying records have been independently reviewed.
