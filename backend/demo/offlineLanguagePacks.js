...
const malay = shared({
  ...
  noteHeadings: ['Mengapa ini penting', 'Cara memahami topik ini', 'Semakan kurikulum dan guru'], reasoning: (topic) => `Jawapan yang kukuh menggunakan kosa kata utama, memberikan contoh tepat dan menerangkan kaitan antara contoh dengan ${topic}.`, curriculumCheck: (framework, source) => `Konteks rangka kerja: ${framework}. ${source}. Sahkan standard tepat, fakta tempatan dan bahasa yang mudah diakses sebelum digunakan di kelas.`, studyPlan: (topic) => `${topic} — Pelan ulang kaji 4 langkah`, schedule: ['Kosa kata dan pengetahuan sedia ada', 'Model guru dengan bukti', 'Latihan penjelasan berpandu', 'Aplikasi dan refleksi kendiri'],
});
